...
The current audit records three passed, four partial, and one failed gate. Same-content overlap advances from fail to partial: a paired scaffold reuses one target identity across binding and access (`audits/stage8_integrated_content_scaffold.json`), and disjoint-split directional effects are seed-robust against permuted and split-state nulls (`audits/stage8_integrated_content_directional_multiseed.json`). Removing forced sharing reveals an important boundary. Ordinary joint supervision yields `0.00` transfer and decreases a near-closed routing gate; severe stochastic loss of the private access lane (`0.95` dropout for 60 epochs) instead raises the unsupervised route from `0.050` to `0.201` and produces `0.937` held-out directional transfer versus `0.00` under both nulls (`audits/stage8_task_induced_routing_sweep.json`). Thus task pressure can induce overlap without a gate or overlap label, but the result is single-seed and depends on severe engineered robustness pressure. The remaining failed gate is a genuinely different benchmark. Next are seed replication of both routing endpoints, a less benchmark-specific pressure, and different-task replication.
