...
The current audit records three passed, four partial, and one failed gate. Same-content overlap advances from fail to partial because the explicitly shared assay reuses one identity across binding and access and its disjoint-split directional effects are seed-robust against permuted and split-state nulls (`audits/stage8_integrated_content_directional_multiseed.json`). Forced sharing cannot yet be removed: ordinary joint supervision is null, and a corrected task-pressure audit yields at most `0.021` transfer (`audits/stage8_task_induced_routing_correction.json`). The remaining failed Stage 8 gate is a genuinely different benchmark. The temporal-relay dataset is structurally different, but its first generic-GRU pilot memorizes training event conjunctions: held-out joint accuracy is `0.020` versus `0.015` for the order-destroyed control, and causal non-payload stability is `0.064` (`audits/stage8_temporal_relay_pilot.json`). Different-benchmark replication therefore remains failed. Next is a relational sequence encoder that preserves order while removing entity-identity memorization.
