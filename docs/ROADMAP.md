...
The current audit records three passed, three partial, and two failed gates. The decisive failures are the absence of cross-validated causal overlap for the same internal content across theory-derived families and the absence of replication on a genuinely different benchmark. Branch C and Branch D provide strong bounded results, but their surface/cardinality variants remain within their original synthetic task families and therefore do not satisfy the different-benchmark gate. A paired integrated-content scaffold supplies the missing unit of analysis: one target identity and initial feature bundle is reused across binding and four access-status transitions, with current attention fixed elsewhere (`audits/stage8_integrated_content_scaffold.json`). Its first trained pilot validates the causal assay: exactly matched shared and split models are both task-perfect, but a binding-state swap coordinates binding and access only through the shared state (`1.00` versus `0.115` joint donor-follow; `audits/stage8_integrated_content_pilot.json`). Because that bottleneck is architecturally imposed and the result is single-seed, the Stage 8 gate remains open. Next are multi-seed replication, a direction learned on disjoint data rather than a direct state swap, and replication on a structurally different task.
