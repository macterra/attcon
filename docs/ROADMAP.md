...
The current audit records three passed, four partial, and one failed gate. Same-content overlap advances from fail to partial: a paired scaffold reuses one identity across binding and access, and disjoint-split directional effects are seed-robust against permuted and split-state nulls (`audits/stage8_integrated_content_directional_multiseed.json`). Removing forced sharing reveals an important boundary. Ordinary joint supervision produces `0.00` maximum transfer over three fresh seeds. Severe stochastic loss of the private access lane (`0.95` dropout, 60 epochs) produces `0.910` minimum transfer and passes both causal-null advantages in every run, but the full endpoint passes only 1/3 seeds: one blocked control is not task-viable enough and one routing coefficient narrowly misses its floor (`audits/stage8_task_induced_routing_multiseed.json`). Task pressure therefore induces a robust causal influence without a gate or overlap label, but full routing support remains seed-fragile and false. The remaining failed Stage 8 gate is a genuinely different benchmark. Next are a frozen-threshold duration check, a less benchmark-specific pressure, and different-task replication.
