...
The current audit records three passed, three partial, and two failed gates. The decisive failures are the absence of qualifying cross-family causal overlap for the same internal content and the absence of replication on a genuinely different benchmark. Branch C and Branch D provide strong bounded results, but their surface/cardinality variants remain within their original synthetic task families and therefore do not satisfy the different-benchmark gate. A paired integrated-content scaffold supplies the missing unit of analysis: one target identity and initial feature bundle is reused across binding and four access-status transitions, with current attention fixed elsewhere (`audits/stage8_integrated_content_scaffold.json`). Whole-state causal effects are seed-robust (`audits/stage8_integrated_content_multiseed.json`), and the stricter directional pilot fits value directions only on training states before intervening on held-out bundles. It reaches `1.00` joint binding/access donor-follow with `1.00` non-value stability, versus `0.00` for both permuted-label and exactly matched split-state nulls (`audits/stage8_integrated_content_directional.json`). This establishes cross-validated directional overlap within the engineered assay, but the shared bottleneck remains architecturally imposed rather than independently emerging across qualifying theory families. The Stage 8 gate therefore remains open. Next are multi-seed directional replication, removal of forced sharing, and replication on a structurally different task.
