...
The current audit records three passed, four partial, and one failed gate. Same-content overlap advances from fail to partial because the explicitly shared assay reuses one identity across binding and access and its disjoint-split directional effects are seed-robust against permuted and split-state nulls (`audits/stage8_integrated_content_directional_multiseed.json`). Forced sharing cannot yet be removed: ordinary joint supervision is null, and a corrected task-pressure audit yields at most `0.021` transfer (`audits/stage8_task_induced_routing_correction.json`). The remaining failed Stage 8 gate is a genuinely different benchmark. A temporal-relay scaffold now passes all seven dataset invariants across 2,048 paired ordered streams / 8,192 cases. It replaces spatial object selection with repeated-entity updates, last-write resolution, time-operation-payload binding, and delayed access (`audits/stage8_temporal_relay_scaffold.json`). This is not replication until trained sequence and order-destroying controls reproduce the causal result. Next are that trained audit and a naturally coupled resource constraint.
