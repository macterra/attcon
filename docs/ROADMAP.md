...
The current audit records three passed, four partial, and one failed gate. Same-content overlap advances from fail to partial because the explicitly shared assay reuses one identity across binding and access and its disjoint-split directional effects are seed-robust against permuted and split-state nulls (`audits/stage8_integrated_content_directional_multiseed.json`). Forced sharing cannot yet be removed: ordinary joint supervision is null, and a corrected task-pressure audit yields at most `0.021` transfer (`audits/stage8_task_induced_routing_correction.json`). The remaining failed gate, a genuinely different benchmark, now has a positive pilot. On temporal relay, the generic GRU memorizes (`0.020` held-out joint), but a relational sequence encoder clears all ten gates: `1.00` shared/split held-out joint accuracy, `0.041` order-destroyed, `1.00` causal coordination, `0.00` nulls, and `0.998` non-payload stability (`audits/stage8_temporal_relay_relational_pilot.json`). The gate remains failed until fresh-seed replication; the current result is single-seed and explicitly shared.
