...
The current audit records three passed, five partial, and zero failed gates. Same-content overlap is partial because the explicitly shared integrated assay is seed-robust, but forced sharing cannot be removed: ordinary joint supervision is null, and corrected task pressure yields at most `0.021` transfer (`audits/stage8_task_induced_routing_correction.json`). Different-benchmark replication also advances to partial. On temporal relay, the generic GRU memorizes (`0.020` held-out joint), while the relational sequence encoder clears all ten gates in all three fresh runs. Every minimum task, causal, null-advantage, and stability metric is `1.00`, and minimum advantage over the order-destroyed control is `0.934` (`audits/stage8_temporal_relay_multiseed.json`). This is seed-robust engineering transfer to a structurally different task, but query matching and shared state remain explicit. Next are a different sequence architecture and removal of forced sharing on both benchmarks.
