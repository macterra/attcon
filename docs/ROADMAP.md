...
The current audit records three passed, four partial, and one failed gate. Same-content overlap advances from fail to partial: a paired scaffold reuses one identity across binding and access, and disjoint-split directional effects are seed-robust against permuted and split-state nulls (`audits/stage8_integrated_content_directional_multiseed.json`). Removing forced sharing reveals an important boundary. Ordinary joint supervision produces `0.00` maximum transfer over three seeds. Under severe private-lane loss (`0.95` dropout), a frozen-threshold duration check raises full support from 1/3 at 60 epochs to 2/3 at 90 (`audits/stage8_task_induced_routing_duration.json`). Every routing and causal-overlap gate passes every 90-epoch run (minimum route `0.207`, transfer/null advantages `0.928`); only one blocked control misses task viability at `0.889`. Task pressure therefore induces robust causal influence without a gate or overlap label, but full support remains false. The remaining failed Stage 8 gate is a genuinely different benchmark. Next are a viability-first curriculum, a less benchmark-specific pressure, and different-task replication.
