...
The current audit records three passed, four partial, and one failed gate. Same-content overlap advances from fail to partial: a paired integrated-content scaffold reuses one target identity and initial feature bundle across binding and four access-status transitions (`audits/stage8_integrated_content_scaffold.json`), whole-state effects are seed-robust (`audits/stage8_integrated_content_multiseed.json`), and the stricter disjoint-split directional audit passes all ten gates in all three fresh seed runs (`audits/stage8_integrated_content_directional_multiseed.json`). Every minimum task, donor-follow, non-value stability, permuted-null advantage, and split-null advantage metric is `1.00`. This establishes cross-validated, seed-robust directional overlap within the engineered assay, but the shared bottleneck remains architecturally imposed rather than independently emerging across qualifying theory families. The remaining failed gate is replication on a genuinely different benchmark; Branch C/D surface variants preserve their synthetic task families and do not count. Next are removal of forced sharing and replication on a structurally different task.
