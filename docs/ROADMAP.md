...
The current audit records three passed, three partial, and two failed gates. The decisive failures are the absence of cross-validated causal overlap for the same internal content across theory-derived families and the absence of replication on a genuinely different benchmark. Branch C and Branch D provide strong bounded results, but their surface/cardinality variants remain within their original synthetic task families and therefore do not satisfy the different-benchmark gate. A paired integrated-content scaffold now supplies the missing unit of analysis: one target identity and initial feature bundle is reused across binding and four access-status transitions, with current attention fixed elsewhere and a shared-state perturbation point reserved (`audits/stage8_integrated_content_scaffold.json`). All dataset gates pass, but this is not causal evidence. The next decisive experiment is the trained shared-state model with split-state and identity-destroying controls, followed by replication on a structurally different task.
