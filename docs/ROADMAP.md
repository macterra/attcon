...
The current audit records three passed, four partial, and one failed gate. Same-content overlap advances from fail to partial because the explicitly shared assay reuses one identity across binding and access and its disjoint-split directional effects are seed-robust against permuted and split-state nulls (`audits/stage8_integrated_content_directional_multiseed.json`). Forced sharing cannot yet be removed. Ordinary joint supervision produces `0.00` transfer in the neutral dual-lane model. An apparent positive result under severe private-lane dropout was traced to inverted-dropout scaling: the surviving lane was multiplied by `20×` during training but not evaluation. Corrected zero-or-normal occlusion leaves both models task-perfect yet yields at most `0.021` transfer; a viability-first curriculum yields `0.001` (`audits/stage8_task_induced_routing_correction.json`). Task-induced routing is therefore unsupported. The remaining failed Stage 8 gate is a genuinely different benchmark. Next are a naturally coupled task or resource constraint with matched train/evaluation scaling and different-task replication.
