...
The current audit records three passed, three partial, and two failed gates. The decisive failures are the absence of cross-validated causal overlap for the same internal content across theory-derived families and the absence of replication on a genuinely different benchmark. Branch C and Branch D provide strong bounded results, but their surface/cardinality variants remain within their original synthetic task families and therefore do not satisfy the different-benchmark gate. A paired integrated-content scaffold supplies the missing unit of analysis: one target identity and initial feature bundle is reused across binding and four access-status transitions, with current attention fixed elsewhere (`audits/stage8_integrated_content_scaffold.json`). Its causal assay is seed-robust at the engineering level: all ten gates pass on three fresh data/model-seed pairs, with `1.00` minimum shared task and donor-follow metrics and `0.890` minimum coordination advantage over an exactly matched split-state null (`audits/stage8_integrated_content_multiseed.json`). Because the bottleneck is architecturally imposed and the intervention replaces the entire state, the Stage 8 gate remains open. Next are direction-limited interventions learned on disjoint data and replication on a structurally different task.
