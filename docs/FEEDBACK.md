...
- **Pass thresholds are unprincipled.** Now configurable with non-zero defaults (e.g., `min_cell_bce_advantage: 0.01`, `min_accuracy_drop: 0.02`) and `nonnegative_directional_effect` / `probe_effect_positive` flags reported alongside the stricter `passed`. Real improvement, but the thresholds are still hardcoded constants rather than empirically calibrated. Suggest a small noise-floor routine that runs each probe against shuffled labels and uses the 95th-percentile spurious advantage as the threshold.
  - **Addressed (post-rehab, PR #11 and follow-up audits).** `noise_floor_metrics` runs the strong Stage 6A report-signal probes against permuted labels and gates `structured_reportability` on the resulting p95 floor. Stage 6B now independently permutes train/test labels for every gated matched-capacity binary comparison and requires both accuracy and positive-recall advantages to clear their empirical floors. `run_latent_content_noise_floor` applies the same discipline to Stage 7 latent content and supplies the pilot's primary `content_supported` gate; bare directional results are retained only as positive-evidence diagnostics. Stage 4B's causal follow-up instead uses a fitted-direction effect against 32 matched random directions at four intervention scales, which is the appropriate empirical null for that intervention claim.
