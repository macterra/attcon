...
- [~] Add Branch F broadcast/ignition experiments with coordinated intervention tests. The first pilot (`audits/branch_f_broadcast_pilot.json`) compares a shared bottleneck with an exactly parameter-matched private-shortcut upper bound (12,309 parameters each). Both reach `1.00` held-out accuracy on all six consumers, but shared onset accuracy/alignment is `1.00` versus private `0.674`/`0.557`. Zeroing shared state drops broad-consumer accuracy by `0.900`; zeroing one private route drops the five-consumer mean by `0.183`, producing a `0.717` coordinated-ablation advantage. Shared content swaps drive all broad consumers to the donor target at `1.00` while local action remains invariant. Every engineering gate passes. Because consumer outputs and the shared bottleneck are directly imposed, this is engineering support only; spontaneous broadcast remains unsupported.
