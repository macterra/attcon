...
- [~] Check whether any Stage 8-relevant contents show cross-validated causal overlap across branches. The paired integrated-content scaffold now fixes one target identity and initial feature bundle across binding and all four access-status transitions (`audits/stage8_integrated_content_scaffold.json`; 2,048 groups / 8,192 cases, all seven invariant gates pass). This establishes the unit of analysis, not causal overlap. Next: train the shared-state model and split-state / identity-destroying controls, then run bidirectional content-state swaps and perturbations on held-out bundles.
