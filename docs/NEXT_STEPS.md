...
- [x] Add Branch D counterfactual-access experiments for non-current but query-available contents. The first unstructured GRU pilot (`audits/branch_d_access_pilot.json`) is a decisive negative control: `1.00` training accuracy but `0.00` across all 3,228 held-out query/value pairs despite complete individual key/value coverage. The prespecified relational follow-up (`audits/branch_d_access_relational.json`) uses query-key addressing while forcing remembered values through recurrent state; it receives no explicit cache at report time. Under the same split, thresholds, and exactly parameter-matched no-cache control, it reaches `1.00` on all four access states. The no-cache route is `0.00` on previous-memory/counterfactual cases; target-cache erasure drops accuracy by `1.00`; and conflicting-observation changes preserve the cache answer at `1.00`. Every frozen gate passes, giving Branch D bounded support. Multi-seed and alternate relational-architecture replication remain before robust support.
