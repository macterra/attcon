...
- [~] Check whether any Stage 8-relevant contents show cross-validated causal overlap across branches. The paired scaffold fixes one target identity and initial feature bundle across binding and all four access-status transitions (`audits/stage8_integrated_content_scaffold.json`; 2,048 groups / 8,192 cases, all seven invariant gates pass). The first trained pilot (`audits/stage8_integrated_content_pilot.json`) clears all ten engineering gates: exactly parameter-matched shared and split models both reach `1.00` held-out joint task accuracy; the pooled identity control reaches `0.011`; and the same binding-state swap produces `1.00` joint donor-follow in the shared model versus `0.115` in the split control. This validates the assay, but the shared bottleneck is imposed and the run is single-seed. Next: multi-seed replication and a content direction learned on disjoint data rather than direct state replacement.
