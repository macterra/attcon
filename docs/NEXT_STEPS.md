...
- [~] Check whether any Stage 8-relevant contents show cross-validated causal overlap across branches. The paired scaffold fixes one target identity and initial feature bundle across binding and all four access-status transitions (`audits/stage8_integrated_content_scaffold.json`; 2,048 groups / 8,192 cases, all seven invariant gates pass). The trained causal assay is now multi-seed robust (`audits/stage8_integrated_content_multiseed.json`): all ten gates pass on three fresh data/model-seed pairs, all minimum shared task and donor-follow metrics are `1.00`, minimum binding advantage over pooling is `0.980`, and minimum coordination advantage over the split null is `0.890`. This is engineering support only because the shared bottleneck is imposed and the intervention replaces the entire state. Next: learn content directions on disjoint training representations and test direction-limited interventions against permuted-label and split-state nulls.
