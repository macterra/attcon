...
- [~] Replicate supported claims on a second benchmark with different task structure. The temporal-relay scaffold passes all seven invariants over 2,048 paired ordered streams / 8,192 cases (`audits/stage8_temporal_relay_scaffold.json`). Its first generic-GRU pilot is a decisive negative control (`audits/stage8_temporal_relay_pilot.json`): training loss reaches `0.101`, but held-out joint accuracy is `0.020` versus `0.015` order-destroyed, and directional non-payload stability is only `0.064`. The architecture memorizes event conjunctions. Next: relationally mask events by query equality before recurrence, retaining order while removing entity-identity memorization.
