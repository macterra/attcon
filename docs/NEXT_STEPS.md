...
- [~] Check whether any Stage 8-relevant contents show cross-validated causal overlap across branches. Whole-state swaps and stricter disjoint-split directions are multi-seed robust (`audits/stage8_integrated_content_multiseed.json`, `audits/stage8_integrated_content_directional_multiseed.json`). Removing forced sharing produces a useful boundary: ordinary joint supervision yields `0.00` maximum transfer over three seeds. Under `0.95` private-lane dropout, extending training from 60 to 90 epochs raises full support from 1/3 to 2/3 seeds (`audits/stage8_task_induced_routing_duration.json`). All routing and causal-overlap gates pass every run (minimum route `0.207`, transfer/null advantages `0.928`); the only miss is a blocked control at `0.889` versus the `0.90` viability floor. Full support remains false. Next: pretrain both routes to task viability before applying pressure, then replace dropout with a less benchmark-specific demand.
