...
- [x] Add Branch C unity/binding experiments with bound-content probes and intervention tests. The original pilot (`audits/branch_c_binding_pilot.json`) and frozen-gate surface replication (`audits/branch_c_binding_surface_variant.json`) reach `1.00` joint accuracy, lure rejection, and coherent intervention scores on 3,288 and 4,162 held-out conjunctions. Capacity-advantaged pooled baselines reach `0.015`/`0.511` and `0.008`/`0.512` joint/lure performance. The surface replication changes the attribute semantics, every non-binary vocabulary size, object count, data seed, and model seed. The six-run robustness audit (`audits/branch_c_binding_multiseed.json`) adds three independent data/model-seed pairs per variant; all six pass every frozen gate, with `1.00` minimum integrated joint, lure, and intervention scores. Branch C therefore has bounded, seed-robust support. A structurally different binding-model family remains necessary for robust support.
