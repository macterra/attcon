...
- [x] Add Branch D counterfactual-access experiments for non-current but query-available contents. The unstructured GRU pilot (`audits/branch_d_access_pilot.json`) is a decisive negative control: `1.00` training accuracy but `0.00` across all 3,228 held-out query/value pairs. The relational follow-up (`audits/branch_d_access_relational.json`) uses query-key addressing while forcing values through recurrent state and never exposing the explicit cache to the report decoder. It reaches `1.00` on every access state; the exactly matched no-cache route is `0.00` on memory/tension; and both causal interventions score `1.00`. The three-run robustness audit (`audits/branch_d_access_multiseed.json`) repeats this with fresh data/model seeds: every gate passes in every run, with `1.00` minimum accuracy, memory advantage, cache-erasure drop, and observation-conflict retention. Branch D has bounded, seed-robust support. An alternate relational architecture and different surface-task variant remain before robust support.
