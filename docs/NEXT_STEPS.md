...
- [x] Add Branch C unity/binding experiments with bound-content probes and intervention tests. The original pilot (`audits/branch_c_binding_pilot.json`) and a frozen-gate surface replication (`audits/branch_c_binding_surface_variant.json`) evaluate 3,288 and 4,162 held-out conjunctions. The shared-selection route reaches `1.00` joint accuracy, lure rejection, and coherent intervention scores in both. Capacity-advantaged pooled baselines reach `0.015`/`0.511` and `0.008`/`0.512` joint/lure performance. The replication changes grid and object counts, every non-binary vocabulary size, semantic surfaces (grid/category/digit/cue/inspection to tile/shape/texture/color/illumination), data seed, and model seed while retaining the predeclared thresholds. `audits/branch_c_binding_replication.json` therefore records bounded Branch C support. Robustness still requires a multi-seed audit and a model family without the same architecturally imposed selector.
