...
- [~] Check whether any Stage 8-relevant contents show cross-validated causal overlap across branches. Whole-state swaps and stricter disjoint-split directions are multi-seed robust (`audits/stage8_integrated_content_multiseed.json`, `audits/stage8_integrated_content_directional_multiseed.json`). Removing forced sharing produces a useful boundary. Ordinary joint supervision yields `0.00` maximum transfer over three fresh seeds. Severe private-lane robustness pressure (`0.95` dropout, 60 epochs) produces `0.910` minimum directional transfer, and both permuted and blocked-null advantages pass in all runs—but the full frozen endpoint passes only 1/3 seeds because one blocked control misses task viability (`0.860`) and one learned coefficient narrowly misses its floor (`0.197`; `audits/stage8_task_induced_routing_multiseed.json`). Thus causal influence is seed-robust but full task-induced routing support is false. Next: determine whether longer optimization clears the two bottlenecks without changing thresholds, then replace dropout with a less benchmark-specific robustness demand.
