...
- [~] Add Branch D counterfactual-access experiments for non-current but query-available contents. The first trained pilot (`audits/branch_d_access_pilot.json`) compresses access events through a GRU, compares an exactly parameter-matched no-cache GRU, and applies cache-erasure and observation-conflict interventions. It is a decisive combinatorial-generalization failure: the internal route reaches `1.00` accuracy on 13,156 training cases but `0.00` on all 3,228 held-out query/value pairs across all four access states. Every held-out key and answer is individually represented in training and no held-out pair leaks, so the model memorized conjunctions rather than learning flexible retrieval. All predeclared gates fail; Branch D remains unsupported. Next: test a relational key/value architecture under the same frozen split and gates, retaining this result as the unstructured-GRU negative control.
