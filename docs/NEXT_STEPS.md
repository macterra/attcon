...
- [~] Replicate supported claims on a second benchmark with different task structure. The temporal-relay scaffold passes all seven invariants over 2,048 paired ordered streams / 8,192 cases (`audits/stage8_temporal_relay_scaffold.json`). It replaces spatial object-set selection with repeated-entity event updates, last-write resolution, time-operation-payload binding, and delayed live/archive/conflict/missing access. This is a structurally different dataset, not a replication result. Next: train sequence and order-destroying controls and repeat the disjoint directional intervention on held-out event bundles.
