...
- [~] Add Branch C unity/binding experiments with bound-content probes and intervention tests. The first powered pilot (`audits/branch_c_binding_pilot.json`) trains on 13,096 conjunction-training scenes and evaluates 3,288 untouched held-out conjunctions. A single shared selector feeding all four attribute heads reaches `1.00` held-out joint accuracy and lure rejection, compared with `0.015` and `0.511` for a pooled independent-feature baseline with 13% more parameters. Target-type edits are followed while other bound fields remain stable (`1.00`), and non-target edits leave every answer invariant (`1.00`). All predeclared pilot gates pass. Branch C remains partial until this replicates on a benchmark variant with different surface attributes.
