...
- [~] Check whether any Stage 8-relevant contents show cross-validated causal overlap across branches. The paired scaffold fixes one target identity and initial feature bundle across binding and all four access-status transitions (`audits/stage8_integrated_content_scaffold.json`; 2,048 groups / 8,192 cases, all seven invariant gates pass). Whole-state swaps and stricter disjoint-split directions are multi-seed robust (`audits/stage8_integrated_content_multiseed.json`, `audits/stage8_integrated_content_directional_multiseed.json`). Removing forced sharing produces a useful boundary (`audits/stage8_task_induced_routing_sweep.json`): ordinary joint supervision yields `0.00` transfer and a shrinking gate; severe private-lane robustness pressure (`0.95` dropout, 60 epochs) raises the learned route from `0.050` to `0.201` and produces `0.937` held-out directional transfer versus `0.00` permuted and blocked nulls. This is task-induced rather than directly supervised, but remains single-seed and depends on severe engineered pressure. Next: replicate the positive endpoint and zero-pressure falsifier across fresh seeds, then replace dropout with a less benchmark-specific robustness demand.
