...
The current roadmap treats Stages 1 through 3 as the sequential foundation, then splits into parallel lines of work around engineered self-state tracking, stronger learned self-modeling, and flexible reallocation under changed priorities. After structured reportability, the repo now includes a bounded Stage 7 route: a local calibrated reporter reads opaque tokenized internal state and emits faithful natural-language-shaped reports. The powered external-LLM variant has now been evaluated with a negative result; the VLM variant remains follow-up work.
...
- **Stage 7** (faithful NL reportability): supported for the local calibrated opaque-token reporter, capacity audit passes; the VLM route remains open. Sharper caveat: the local decoder reads the scored content fields from attended-content token bases the renderer fills directly from the model's attended content (a schema-aware structural round-trip), so consistent token-remapping and held-out-combination anti-memorization tests do not bite it. A **latent-only decoder** (`run_latent_only_report_mode`) that recovers content from an opaque quantised view of internal state alone is now implemented and runs in CI to make those faithfulness tests meaningful, but on the current checkpoint it does **not** clear the bar (marginal, non-robust current-content advantage; no remembered/counterfactual recovery; `content_supported = false`), so the bounded Stage 7 claim still rests on the round-trip reporter. The powered external GPT-5 mini audit is also negative: across default, cue-switch, and intervened slices, latent-only current-content joint accuracy is `0/12`, `1/12`, and `0/12`, never beating the same model on observation-only prompts; remembered and full-content joint accuracy are `0/12` throughout. See `audits/stage7_external_llm_powered_content_memory_v3.json`.
