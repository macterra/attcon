...
- **Branch D** (counterfactual access): **bounded, seed-robust support with an architecture-specific boundary**. The unstructured GRU memorizes training conjunctions (`1.00`) and scores `0.00` on all 3,228 held-out query/value pairs. Under identical frozen gates, the relational recurrent route reaches `1.00` across all access states while its exactly matched no-cache control scores `0.00` on memory/tension. Cache erasure and observation-conflict interventions are perfect. Three fresh data/model-seed pairs repeat every result with a `1.00` minimum on every headline and causal metric. The decoder never receives the explicit cache. Robustness still needs another relational architecture and surface-task variant. See `audits/branch_d_access_multiseed.json`.
