...
- **Branch C** (unity / binding): **bounded support** across two synthetic surface variants. On 3,288 original and 4,162 surface-v2 held-out conjunctions, one shared object selector feeding every attribute head reaches `1.00` joint accuracy and lure rejection in both variants. Capacity-advantaged pooled baselines reach `0.015`/`0.511` and `0.008`/`0.512`; target and non-target interventions are fully coherent. All frozen gates pass after changing the attribute semantics, cardinalities, data seed, and model seed. Boundary: the binding bottleneck is architecturally enforced, and robustness still needs multiple training seeds and a different binding-model family. See `audits/branch_c_binding_replication.json`.
