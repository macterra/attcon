...
- **Branch D** (counterfactual access): implemented pilot, substantive negative result. An exactly matched GRU access/no-cache comparison reaches `1.00` training accuracy for the access route but `0.00` on all 3,228 held-out query/value conjunctions, despite complete individual key and answer coverage and zero pair leakage. Every predeclared gate fails. This exposes conjunction memorization rather than flexible access and keeps Branch D unsupported. See `audits/branch_d_access_pilot.json`.
