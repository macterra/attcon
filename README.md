...
- **Stage 8** (multi-theory convergence): **not met**. The current artifact-level audit records three pass, four partial, and one failed gate. Same-content causal overlap advances from fail to partial because the integrated directional assay is cross-validated and seed-robust, but its shared bottleneck is imposed rather than independently emerging across qualifying families. The decisive failure is still absence of a genuinely different benchmark. Surface/cardinality variants do not count, and Branch E/F engineering results remain excluded from theoretical-family counts. See `audits/stage8_convergence_current.json`.
...
- **Stage 8 temporal-relay relational pilot**: all ten gates pass on the structurally different ordered task. Query equality is resolved before recurrence, removing entity-identity memorization while retaining chronology. The shared and split models reach `1.00` held-out joint accuracy; the exactly matched order-destroyed control reaches `0.041`. A training-fitted payload direction produces `1.00` joint binding/access donor-follow with `0.998` time/operation stability, versus `0.00` for permuted-label and split-route nulls. This is a single-seed engineered cross-benchmark pilot, not robust replication yet. See `audits/stage8_temporal_relay_relational_pilot.json`.
