...
- **Stage 8 task-induced routing pilot**: ordinary joint supervision is a clean negative result—the learnable route shrinks from `0.050` to `0.040` and directional transfer stays `0.00`. Under stochastic private-access-lane loss, transfer increases with pressure; the only fully passing endpoint is `0.95` dropout trained for 60 epochs. There the unsupervised route rises to `0.201`, held-out joint task accuracy is `1.00`, directional binding-to-access transfer is `0.937`, and both permuted-label and blocked-routing transfer are `0.00`. This is a single-seed result under severe engineered robustness pressure, not spontaneous convergence. See `audits/stage8_task_induced_routing_sweep.json`.
