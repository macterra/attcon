...
- **Stage 8 directional overlap pilot**: all ten gates pass for a more selective, disjoint-split intervention. Value-content directions are fitted only from training states and applied to held-out content bundles. The true direction moves binding and accessible access to the new value jointly at `1.00`, while preserving location/type and unavailable-status behavior at `1.00`; the permuted-label direction and exactly matched split-state access path both score `0.00` joint donor-follow. This rules out whole-state replacement and arbitrary directions within the engineered assay. The shared architecture is still imposed, so this is not independent theory-family convergence. See `audits/stage8_integrated_content_directional.json`.
