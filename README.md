...
- **Branch D** (counterfactual access): **bounded support with an architecture-specific boundary**. The unstructured GRU memorizes training conjunctions (`1.00`) and scores `0.00` on all 3,228 held-out query/value pairs. Under the identical frozen split and gates, a relational recurrent route reaches `1.00` on unavailable, merely visible, previously attended, and counterfactual-tension cases; its exactly matched no-cache control scores `0.00` on the memory/tension subset. Cache erasure causes a `1.00` accuracy drop, while conflicting-observation changes leave cached answers intact at `1.00`. The report decoder does not receive the explicit cache. Robustness still needs multiple seeds and another relational architecture. See `audits/branch_d_access_relational.json`.
