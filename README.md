...
- **Branch F** (broadcast / ignition): **engineering support only; spontaneous broadcast unsupported**. Shared and private-shortcut models are exactly matched at 12,309 parameters and both solve all six held-out consumers at `1.00`. Only the shared model has `1.00` aligned onset. Zeroing its broadcast state drops broad accuracy by `0.900`; zeroing one private route drops the five-consumer mean by `0.183` (`0.717` coordination advantage). Content-state swaps move every broad output to the donor at `1.00` while local action is unchanged. Because outputs and the bottleneck are explicitly engineered, Branch F remains theoretically unsupported. See `audits/branch_f_broadcast_pilot.json`.
