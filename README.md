...
- **Branch D** (counterfactual access): benchmark scaffold complete; no positive model evidence yet. The 4,096-episode audit balances unavailable, merely visible, previously attended, and counterfactually accessible targets; contains 831 held-out query/value pairs; holds fixation fixed through every query switch; and puts the visible decoy in tension with the cached answer in every counterfactual case. Scene-only controls score `1.00` on merely visible/unavailable cases and `0.00` on previous-memory/tension cases, establishing the intended discrimination. See `audits/branch_d_access_dataset.json`.
