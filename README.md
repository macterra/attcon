...
- **Stage 8 temporal-relay relational pilot**: all ten gates pass in all three fresh seed runs on the structurally different ordered task. Query equality is resolved before recurrence, removing entity-identity memorization while retaining chronology. Every minimum shared/split task, directional, null-advantage, and non-payload stability metric is `1.00`; minimum advantage over the exactly matched order-destroyed control is `0.934`. This is seed-robust engineered cross-benchmark transfer, but relational matching and state sharing remain explicit. See `audits/stage8_temporal_relay_multiseed.json`.
