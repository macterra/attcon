...
- **Branch C** (unity / binding): positive single-benchmark pilot evidence, but not yet supported. On 3,288 held-out target conjunctions, one shared object selector feeding every attribute head reaches `1.00` joint accuracy and `1.00` false-binding-lure rejection, versus `0.015` and `0.511` for an object-identity-destroying pooled baseline with 13% more parameters. Target-feature and non-target interventions are fully coherent. The predeclared pilot gates all pass; a different surface-attribute benchmark is still required by the Branch C claim threshold. See `audits/branch_c_binding_pilot.json`.
