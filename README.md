...
- **Stage 8 task-induced routing pilot**: ordinary joint supervision is a clean multi-seed negative result—maximum directional transfer is `0.00`. Under `0.95` stochastic private-access-lane loss for 60 epochs, causal transfer is robust (minimum `0.910`; both permuted and blocked-null advantages pass in all three fresh runs), but the full frozen gate passes only 1/3 seeds. One blocked control falls below the `0.90` task-viability floor (`0.860`) and one learned routing coefficient narrowly misses `0.20` (`0.197`). Thus task pressure reliably induces causal influence, but full task-induced routing support is seed-fragile and remains false. See `audits/stage8_task_induced_routing_multiseed.json`.
