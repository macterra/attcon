...
- **Branch E** (higher-order state): benchmark scaffold complete; no model evidence yet. The 4,092-case audit contains 682 complete counterbalances holding first-order key/value content fixed across fresh-current, fresh-memory, inferred, unavailable, stale-access, and wrong-access states, with 816 held-out content/status conjunctions. Fresh-current and wrong-access observations are identical by construction. Content-only and observation-only status oracles are capped at `0.167` and `0.333`. See `audits/branch_e_higher_order_dataset.json`.
