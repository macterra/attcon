...
- **Branch C** (unity / binding): **bounded, seed-robust support** across two synthetic surface variants. On 3,288 original and 4,162 surface-v2 held-out conjunctions, one shared object selector feeding every attribute head reaches `1.00` joint accuracy and lure rejection in both variants. Capacity-advantaged pooled baselines reach `0.015`/`0.511` and `0.008`/`0.512`; target and non-target interventions are fully coherent. A six-run audit (three independent data/model-seed pairs on each variant) passes every frozen gate with minimum integrated joint, lure, and intervention scores of `1.00`. Boundary: the binding bottleneck is architecturally enforced, so a different binding-model family is still required for robust support. See `audits/branch_c_binding_multiseed.json`.
