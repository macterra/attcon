...
- **Stage 8 task-induced routing pilot**: ordinary joint supervision is a clean multi-seed negative result—maximum directional transfer is `0.00`. Under `0.95` stochastic private-access-lane loss, causal transfer is robust. Extending training from 60 to 90 epochs raises full support from 1/3 to 2/3 seeds: every routing and causal-overlap gate passes all runs (minimum route `0.207`, transfer/null advantages `0.928`), but one blocked control scores `0.889`, narrowly below the `0.90` task-viability floor. Full task-induced routing support therefore remains false; the next design establishes control viability before pressure. See `audits/stage8_task_induced_routing_duration.json`.
