...
- **Stage 7 latent-memory replication**: under explicit v3 content-memory regularization,
  remembered-content recovery clears a permuted-label p95 floor across all four training seeds and
  all tested slice/interface cells (mean advantages `+0.41..+0.64` versus p95 `~0.08`). The strict
  `content_only` joint advantage is seed-fragile (full on two seeds, partial on one, absent on one),
  so this is robust engineered memory reportability rather than spontaneous or strict Stage 7
  support. See `audits/stage7_latent_noise_floor_v3_seed*.json`.
...
- **Stage 8** (multi-theory convergence): **not met**. The current artifact-level audit records three pass, five partial, and zero failed gates. Same-content causal overlap and different-benchmark transfer now each have cross-validated, seed-robust engineering assays. Neither is upgraded to pass because relational matching and shared-state mechanisms are imposed rather than independently emerging across qualifying families; Branch E/F engineering results remain excluded from theory-family counts. See `audits/stage8_convergence_current.json`.
...
- **Stage 8 integrated-content pilot**: the causal-overlap assay is seed-robust at the engineering level. Exactly parameter-matched shared-state and split-state models (22,003 parameters each) both reach `1.00` held-out binding/access joint accuracy, while identity pooling collapses binding. A binding-state swap makes both binding and accessible access outputs follow the donor only through the shared state. All ten gates pass on three fresh data/model-seed pairs: every minimum shared task and donor-follow metric is `1.00`, minimum binding advantage over pooling is `0.980`, and minimum coordination advantage over the split null is `0.890`. The bottleneck is still imposed and the intervention replaces the whole state, so the Stage 8 same-content gate remains open. See `audits/stage8_integrated_content_multiseed.json`.
- **Stage 8 directional overlap pilot**: all ten gates pass in all three fresh seed runs for a selective, disjoint-split intervention. Value-content directions are fitted only from training states and applied to held-out content bundles. Every minimum task, donor-follow, non-value stability, permuted-null advantage, and split-null advantage metric is `1.00`. This rules out whole-state replacement, arbitrary directions, and seed luck within the engineered assay. The shared architecture is still imposed, so this is not independent theory-family convergence. See `audits/stage8_integrated_content_directional_multiseed.json`.
- **Stage 8 task-induced routing correction**: **unsupported**. Ordinary joint supervision produces `0.00` transfer. The apparent positive result under `0.95` private-lane dropout used inverted-dropout scaling, multiplying the surviving private state by `20×` during training but not evaluation; it is retained as a diagnostic, not support. Corrected zero-or-normal lane occlusion keeps learned and blocked controls task-perfect yet yields at most `0.021` directional transfer; a viability-first curriculum yields `0.001`. Thus learned routing has not emerged. See `audits/stage8_task_induced_routing_correction.json`.
- **Stage 8 temporal-relay scaffold**: all seven dataset gates pass for a structurally different task. Unlike spatial object-set selection, this benchmark uses an ordered event stream with repeated entity updates, requires last-write resolution, binds time/operation/payload, and delays access through live-buffer, archive, conflict, and missing transitions. It contains 2,048 paired streams / 8,192 cases, including 1,628 held-out event bundles. This prepares the different-benchmark test but does not satisfy it until trained sequence and order-destroying controls replicate the causal result. See `audits/stage8_temporal_relay_scaffold.json`.
- **Stage 8 temporal-relay first pilot**: **unsupported**. The generic sequence GRU fits training (`0.101` final loss) but reaches only `0.020` held-out binding/access joint accuracy, essentially matching the order-destroyed control (`0.015`). Although the payload direction moves both payload outputs, time/operation stability is only `0.064`, so the causal result is invalid. This is a useful conjunction-memorization negative control; the next model must use relational query matching before recurrence. See `audits/stage8_temporal_relay_pilot.json`.
- **Stage 8 temporal-relay relational pilot**: all ten gates pass in all three GRU seed runs on the structurally different ordered task. Every minimum task, directional, null-advantage, and non-payload stability metric is `1.00`; minimum advantage over the order-destroyed control is `0.934`. A position-aware relational transformer then passes all ten gates at `1.00` on its first seed, with `0.951` order-destroyed advantage. This is seed-robust GRU and single-seed cross-architecture engineering transfer; relational matching and state sharing remain explicit. See `audits/stage8_temporal_relay_multiseed.json` and `audits/stage8_temporal_relay_transformer_pilot.json`.
